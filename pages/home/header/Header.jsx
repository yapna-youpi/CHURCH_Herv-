...
function Header({ moon, colorTheme, }) {
    ...
    console.log('la valeurs du icon', showIcon)
  return (
    <div className="hero">
        <nav id='nav' className='z-10 w-full h-screen sm:border-r border-slate-400 md:border-r-0 sm:w-5/12 bg-big dark:bg-transparent md:bg-transparent  text-maron font-semibold md:dark:bg-transparent dark:transition-all dark:duration-300 dark:ease-out dark:text-white md:w-full md:flex md:h-20 md:items-center md:justify-around fixed'>
          ...
          <menu className="menu mt-20 pt-2 ml-0 lg:ml-44 md:pt-0 md:mt-0">
            <ul className='md:flex'>
              <li className="liste"><i><HiHome /></i> Home</li>
              <li className="liste"><i><BsCalendar2DateFill /></i> Calendar</li>
              <li className="liste"><i><FaBlogger/></i> Blog</li>
              <li className="liste"><i><FaDonate/> </i> Donate</li>
              <li className="liste"><i>< HiShoppingCart /></i> Shop</li>
              <li className="liste"><i>< AiFillContacts/></i> Contacts</li>
            </ul>
          </menu>
        ...
        { showIcon ?
            <span onClick={() => setShowIcon(!showIcon)} className="bar dark:text-white z-20 absolute top-7 right-7 md:hidden ">
        ...
        <div id="banner" className="banner2 w-12/12 h-screen md:dark:text-red-800">
